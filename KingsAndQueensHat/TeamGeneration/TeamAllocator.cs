...
namespace KingsAndQueensHat.TeamGeneration
{
    ...
    public class TeamAllocator
    {
        ...
        public List<Team> CreateTeams(IPlayerProvider playerProvider)
        {
            ...
            var males = playerProvider.PresentPlayers().Where(p => p.Gender == Gender.Male).ToList().Shuffle();
            var females = playerProvider.PresentPlayers().Where(p => p.Gender == Gender.Female).ToList().Shuffle();
